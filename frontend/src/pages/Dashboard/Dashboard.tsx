...
export default function Dashboard() {
    ...
    const currentCash = useMemo(() => {
        ...
        Object.values(balances).forEach(history => {
            if (history.length > 0) {
                // ⚡ Bolt: Replace O(N log N) sorting with O(N) reduce for getting latest balance
                const last = history.reduce((latest, current) => current.date > latest.date ? current : latest, history[0]);
    ...
    const currentPortfolioValue = useMemo(() => {
        ...
        // ⚡ Bolt: Replace O(N log N) sorting with O(N) reduce for finding latest date
        const latestDate = keys.reduce((latest, current) => current > latest ? current : latest, keys[0]);
