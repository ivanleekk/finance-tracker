...
export default function Dashboard() {
    ...
    const chartData = useMemo(() => {
        ...
        // ⚡ Bolt: Pre-compute maps for O(1) lookups instead of O(N) find/filter inside the loop
        const balancesByDateAndAcc = new Map<string, Map<string, number>>();
        Object.entries(balances).forEach(([accId, history]) => {
            history.forEach(b => {
                if (!balancesByDateAndAcc.has(b.date)) balancesByDateAndAcc.set(b.date, new Map());
        ...
        const rawData = sortedDates.map(date => {
            ...
            if (balancesToday) {
                balancesToday.forEach((val, accId) => accountLatestBalances.set(accId, val));
            }
