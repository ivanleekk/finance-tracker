...
export default function Dashboard() {
    ...
    const chartData = useMemo(() => {
        // ⚡ Bolt: O(N) single-pass aggregation to prevent main-thread bottleneck
        // Pre-group data by date to avoid O(N²) nested array operations in the render loop.
        ...
        snapshots.forEach(s => {
            ...
            snapshotsByDate.set(
                s.date,
                (snapshotsByDate.get(s.date) || 0) + Number(s.current_value_home_currency)
            );
        });
        ...
        const rawData = sortedDates.map(date => {
            ...
            if (balsOnDate) {
                balsOnDate.forEach(({accId, balance}) => {
                    const prev = accountLatestBalances.get(accId) || 0;
                    currentTotalCash += (balance - prev);
