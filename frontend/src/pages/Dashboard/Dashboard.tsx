...
export default function Dashboard() {
    ...
    const currentCash = useMemo(() => {
        ...
        Object.values(balances).forEach(history => {
            if (history.length > 0) {
                const sorted = [...history]// ⚡ Bolt: Optimize date string sorting using standard operators instead of localeCompare
    ...
    const chartData = useMemo(() => {
        ...
        return Array.from(binned.values())// ⚡ Bolt: Optimize date string sorting using standard operators instead of localeCompare
                .sort((a, b) => a.date > b.date ? 1 : a.date < b.date ? -1 : 0);
                ...
                <CardContent>
                    <div className="space-y-4">
                        {transactions.length > 0 ? (
                            transactions.slice(0, 5)// ⚡ Bolt: Optimize date string sorting using standard operators instead of localeCompare
                            .sort((a, b) => b.date > a.date ? 1 : b.date < a.date ? -1 : 0).map((tx) => (
