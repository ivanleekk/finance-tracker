...
export default function Dashboard() {
    ...
    const chartData = useMemo(() => {
        ...
        // Pre-calculate snapshot values by date for O(1) lookup
        ...
        // Pre-group balance updates by date for O(1) lookup
        ...
        Object.entries(balances).forEach(([accId, history]) => {
            history.forEach(b => {
                ...
                balanceUpdatesByDate.get(b.date)!.push({accId, balance: Number(b.balance_home_currency ?? b.balance)});
            });
        });

        const rawData = sortedDates.map(date => {
            // Process balance updates for this date
