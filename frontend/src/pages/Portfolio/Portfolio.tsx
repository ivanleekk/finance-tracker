    ...
    const portfoliosData = useMemo(() => {
        ...
        const binHistory = (history: any[]) => {
            ...
            return Array.from(binned.entries())
                ...
                // ⚡ Bolt: Replace localeCompare with standard relational operators for faster date string comparison
        ...
        const calculateFromSnapshots = (snaps: typeof snapshots) => {
            ...
            // ⚡ Bolt: Replace O(N log N) sort with O(N) reduce for extremum finding
            const keys = Array.from(dailyEquity.keys());
            const latestDate = keys.length > 0 ? keys.reduce((max, current) => max > current ? max : current) : undefined;
