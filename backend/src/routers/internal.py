...
def scheduled_snapshot_job(db: Session = Depends(get_db)):
    try:
        ...
        # Avoid information disclosure by not exposing the full exception details
        print(f"Error in scheduled snapshot job: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An error occurred while processing the scheduled snapshot job."
        )
