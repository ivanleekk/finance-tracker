...
- **Growth charts** (`Support/PortfolioAnalytics.swift`) are built by `equityCurve(snapshots:subPortfolioId:range:now:)` and shared by the Portfolio tab and the sub-portfolio detail screen. Two deliberate divergences from web:
    ...
    - `periodChange`'s fraction is a raw curve-endpoint ratio with **no cash-flow adjustment**, so a recurring contribution still counts as "growth" the same as a market gain — the 1%-base guard above only catches the extreme case, not a household funding a goal $500/month. For the `ALL` range, `SubPortfolioDetailView`/`PortfolioView` swap in `metrics.simpleReturn` (already flow-adjusted the same way TWR/IRR are — see performance.py, issue #256) instead of the naive fraction; the dollar delta stays curve-based either way, since "value went up by $X" is true regardless of where the money came from. Shorter ranges (1M/3M/6M/1Y) still use the naive fraction — extending the fix to them needs range-scoped backend metrics, which isn't wired up yet.
