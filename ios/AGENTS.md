...
- **Staging** is a third build configuration (`Staging`, Release-type — optimised, no `DEBUG`, so no runtime override) selected by the **"FinanceTracker Staging"** scheme. It points at `https://stagfinanceapi.ivanleekaikiat.com` (the `dev`-branch stack, DEPLOYMENT.md §9), uses bundle id `com.ivanlee.financetracker.staging` and display name "Waypoint Stg", so it installs **beside** production with its own Keychain — a staging login can't overwrite the production tokens. `AppConfig.isStaging` reads the `APP_ENVIRONMENT` build setting and drives the orange `StagingBadge` on the login screen and More tab. Run it from Xcode by picking that scheme, or archive it with `-scheme "FinanceTracker Staging"` in the Release-build steps below. Tests stay on the main scheme.
