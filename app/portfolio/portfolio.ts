...
export async function getPortfolioStandardDeviation(request: Request) {
    ...
    for (const symbol of symbols) {
        // fetch and cache forex conversion rate if not already fetched
        const stockCurrency = portfolioData.find((item) => item.symbol === symbol)?.currency || "USD";
        let conversionRate = 1;
        if (stockCurrency !== homeCurrency) {
            if (conversionRates[stockCurrency]) {
                conversionRate = conversionRates[stockCurrency];
            } else {
                const forexQuote = await yahooFinance.quoteSummary(stockCurrency + homeCurrency + "=X");
                if (forexQuote && forexQuote.price) {
                    conversionRate = forexQuote.price.regularMarketPrice!;
                    conversionRates[stockCurrency] = conversionRate;
                }
            }
        } else {
            conversionRate = 1;
    ...
    for (const symbol of symbols) {
        ...
        for (const item of quote.quotes) {
            ...
            homeCurrentPrice *= conversionRates[stockCurrency] || 1;
